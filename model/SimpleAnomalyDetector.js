...
function detect(key, anomaly_file, cf) {
    ...
    console.log(size)
    for (let i = 0; i < size; i++) { // size = 26

        var f1 = cf[i].feature1; //A
        var f2 = cf[i].feature2; //C
        var t1 = dictionary[f1] // t1.length = 2175
        ...
        for(let n = 0; n < t1.length; n++) {
            ...
            // elem = parseFloat(t1[n])
            arr1.push(t1[n])
            //console.log(arr1[n])
            arr2.push(t2[n])
        }

        console.log(arr1[t1.length])
        console.log(arr2[t2.length])

        for (let j = 0; j < t1.length; j++) {

            //  p1 = {x: dictionary[Object.values(dictionary)[f1]][j], 
            //      y: dictionary[Object.values(dictionary)[f2]][j]};
            p1 = {x: arr1[j], y: arr2[j]}
            //console.log(t1)
